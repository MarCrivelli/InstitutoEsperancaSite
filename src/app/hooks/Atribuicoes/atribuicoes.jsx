export const atribuicoes = {
  cachorroTriste: (
    <a
      href="https://www.flaticon.com/free-icons/sad"
      title="sad icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Sad icons created by Freepik - Flaticon
    </a>
  ),

  dinheiro: (
    <a
      href="https://www.flaticon.com/free-icons/business-and-finance"
      title="business and finance icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Business and finance icons created by cah meunang - Flaticon
    </a>
  ),

  doacao: (
    <a
      href="https://www.flaticon.com/free-icons/adopt"
      title="Adopt icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Adopt icons created by Freepik - Flaticon
    </a>
  ),

  adocao: (
    <a
      href="https://www.flaticon.com/free-icons/adopt"
      title="Adopt icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Adopt icons created by Freepik - Flaticon
    </a>
  ),

  colaborador: (
    <a
      href="https://www.flaticon.com/free-icons/collaboration"
      title="collaboration icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Collaboration icons created by Freepik - Flaticon
    </a>
  ),

  instagram: (
    <a
      href="https://www.flaticon.com/free-icons/instagram"
      title="instagram icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Instagram icons created by Grow studio - Flaticon
    </a>
  ),

  facebook: (
    <a
      href="https://www.flaticon.com/free-icons/facebook"
      title="facebook icons"
    >
      Facebook icons created by Enamo Studios - Flaticon
    </a>
  ),

  editar: (
    <a
      href="https://www.flaticon.com/free-icons/edit"
      title="edit icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Edit icons created by Kiranshastry - Flaticon
    </a>
  ),

  relogioVermelho: (
    <a
      href="https://www.flaticon.com/free-icons/calendar"
      title="calendar icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Calendar icons created by Freepik - Flaticon
    </a>
  ),

  dinheiroRoxo: (
    <a
      href="https://www.flaticon.com/free-icons/budget"
      title="budget icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Budget icons created by Freepik - Flaticon
    </a>
  ),

  casaLaranja: (
    <a
      href="https://www.flaticon.com/free-icons/real-estate"
      title="real estate icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Real estate icons created by Smashicons - Flaticon
    </a>
  ),

  pessoaVerde: (
    <a
      href="https://www.flaticon.com/free-icons/good-person"
      title="good person icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Good person icons created by nangicon - Flaticon
    </a>
  ),

  papelAmarelo: (
    <a
      href="https://www.flaticon.com/free-icons/checklist"
      title="checklist icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Checklist icons created by Freepik - Flaticon
    </a>
  ),

  racao: (
    <a
      href="https://www.flaticon.com/free-icons/dog-food"
      title="dog-food icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Dog-food icons created by monkik - Flaticon
    </a>
  ),

  coneVeterinario: (
    <a
      href="https://www.flaticon.com/free-icons/cone-of-shame"
      title="cone of shame icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Cone of shame icons created by Freepik - Flaticon
    </a>
  ),

  coleira: (
    <a
      href="https://www.flaticon.com/free-icons/collar"
      title="collar icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Collar icons created by Freepik - Flaticon
    </a>
  ),

  caminha: (
    <a
      href="https://www.flaticon.com/free-icons/dog"
      title="dog icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Dog icons created by Freepik - Flaticon
    </a>
  ),

  lua: (
    <a
      href="https://www.flaticon.com/free-icons/full-moon"
      title="full moon icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Full moon icons created by vectorsmarket15 - Flaticon
    </a>
  ),

  policial: (
    <a
      href="https://www.flaticon.com/free-icons/police"
      title="police icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Police icons created by Freepik - Flaticon
    </a>
  ),

  binoculo: (
    <a
      href="https://www.flaticon.com/free-icons/find"
      title="find icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Find icons created by Freepik - Flaticon
    </a>
  ),

  atendente: (
    <a
      href="https://www.flaticon.com/free-icons/operator"
      title="operator icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Operator icons created by kliwir art - Flaticon
    </a>
  ),

  arvore: (
    <a
      href="https://www.flaticon.com/free-icons/bonsai"
      title="bonsai icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Bonsai icons created by justicon - Flaticon
    </a>
  ),

  menos: (
    <a
      href="https://www.flaticon.com/free-icons/minus"
      title="minus icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Minus icons created by Freepik - Flaticon
    </a>
  ),

  mais: (
    <a
      href="https://www.flaticon.com/free-icons/plus"
      title="plus icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Plus icons created by Freepik - Flaticon
    </a>
  ),

  doacoes: (
    <a
      href="https://www.flaticon.com/free-icons/donation"
      title="donation icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Donation icons created by Freepik - Flaticon
    </a>
  ),

  engrenagem: (
    <a
      href="https://www.flaticon.com/free-icons/execution"
      title="execution icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Execution icons created by Kalashnyk - Flaticon
    </a>
  ),

  homemConfuso: (
    <a
      href="https://www.flaticon.com/free-icons/confused"
      title="confused icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Confused icons created by Freepik - Flaticon
    </a>
  ),

  sino: (
    <a
      href="https://www.flaticon.com/free-icons/notification"
      title="notification icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Notification icons created by kliwir art - Flaticon
    </a>
  ),

  fechar: (
    <a
      href="https://www.flaticon.com/free-icons/letter-x"
      title="letter x icons"
      target="_blank"
      rel="noopener noreferrer"
    >
      Letter x icons created by Md Tanvirul Haque - Flaticon
    </a>
  ),
};
