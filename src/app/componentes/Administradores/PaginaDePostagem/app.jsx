...
export default function ProgramarPostagem() {
  ...
  const [plataformas, setPlataformas] = useState(["facebook", "instagram"]);
    ...
    const carregarStatusMeta = async () => {
      try {
        const resposta = await fetch(
          `${import.meta.env.VITE_API_URL}/postagens/meta/status`,
          {
            headers: {
        ...
        if (resposta.ok) setStatusMeta(await resposta.json());
      } catch (error) {
        console.error("Erro ao verificar a integração Meta:", error);
      }
    };
    carregarStatusMeta();
  }, []);

  const alternarPlataforma = (plataforma) => {
    setPlataformas((atuais) =>
      atuais.includes(plataforma)
        ? atuais.filter((item) => item !== plataforma)
        : [...atuais, plataforma],
    );
  };
  ...
  const publicar = async () => {
    ...
    if (!legenda.trim() || !plataformas.length) {
      setMensagem("Digite a legenda e selecione ao menos uma rede social.");
      return;
    }
    try {
      setEnviando(true);
      const filtros = {
        idade: filtrosSelecionados.idade.map((opcao) => Number(opcao.value)),
        tipo: filtrosSelecionados.tipoAnimal?.value || "",
        sexo: filtrosSelecionados.sexo?.value || "",
        statusVacinacao: filtrosSelecionados.statusVacinacao?.value || "",
        statusCastracao: filtrosSelecionados.statusCastracao?.value || "",
        statusAdocao: filtrosSelecionados.statusAdocao?.value || "",
        statusMicrochipagem:
          filtrosSelecionados.statusMicrochipagem?.value || "",
        statusVermifugacao: filtrosSelecionados.statusVermifugacao?.value || "",
      };
      const agendadaPara =
        filtrosSelecionados.opcaoPublicacao?.value === "agendar"
          ? filtrosSelecionados.dataPostagem
          : null;
      const resposta = await fetch(
        `${import.meta.env.VITE_API_URL}/postagens`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ legenda, plataformas, filtros, agendadaPara }),
        },
      );
      const resultado = await resposta.json();
      if (!resposta.ok)
        throw new Error(resultado.message || "Erro ao criar postagem.");
      setMensagem(
        resultado.status === "agendada"
          ? "Postagem agendada com sucesso."
          : resultado.status === "publicada"
            ? "Postagem publicada com sucesso."
            : `Postagem salva com status: ${resultado.status}.`,
      );
    } catch (error) {
  ...
  return (
    <div className={styles.fundoPagina}>
      ...
      >
        <g
          transform="translate(453.93861595437744 211.12544562204354)"
          className={styles.oBlob1}
        >
          <path
            id="blob1"
